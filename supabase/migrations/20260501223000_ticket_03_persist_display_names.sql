-- Persist display names for words and families to keep UI labels stable after reload.
alter table public.words
	add column if not exists display_name text;

alter table public.word_family
	add column if not exists display_name text;

update public.words
set display_name = word_id
where display_name is null;

update public.word_family
set display_name = word_family_id
where display_name is null;
...
begin
	...
	loop
		...
		insert into public.words (word_id, display_name, modification_date)
		values (
			v_word_id,
			coalesce(word_item->>'display_name', v_word_id),
	...
	loop
		...
		insert into public.word_family (word_family_id, display_name, modification_date)
		values (
			v_family_id,
			coalesce(family_item->>'display_name', v_family_id),
	...
	loop
		delete from public.word_family_association
		where word_family_association.word_id = delete_word_id;
	...
	loop
		delete from public.word_family_association
		where word_family_association.word_family_id = delete_family_id;
