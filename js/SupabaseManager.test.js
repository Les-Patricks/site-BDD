...
import { fetchFromTable, addInTable, updateInTable, deleteFromTable } from './SupabaseManager.js';
...
describe('fetchFromTable', () => {
    ...
    it('retourne bien les données du mock', async () => {
        mockResult = { data: [{ word_id: 'chat' }, { word_id: 'chien' }], error: null };
        const data = await fetchFromTable('words', '*');
        expect(data).toEqual([{ word_id: 'chat' }, { word_id: 'chien' }]);
    });

    it('lance une erreur si la table est vide', async () => {
        await expect(fetchFromTable('')).rejects.toThrow("fetchFromTable: 'table' est requis.");
    });
...
describe('addInTable', () => {
    it('insère sans erreur', async () => {
        await expect(addInTable('words', { word_id: 'Test' })).resolves.not.toThrow();
    });

    it('appelle insert() avec les bonnes valeurs', async () => {
        await addInTable('words', { word_id: 'Test' });
        expect(queryMock.insert).toHaveBeenCalledWith([{ word_id: 'Test' }]);
    });
    ...
    it('propage une erreur Supabase', async () => {
        ...
        await expect(addInTable('words', { word_id: 'Test' })).rejects.toThrow('Insert error');
    });
});
...
describe('deleteFromTable', () => {
    ...
    it('propage une erreur Supabase', async () => {
        ...
        await expect(deleteFromTable('words', { where: 'eq', col: 'word_id', value: 'Test' }))
...
describe('updateInTable', () => {
    it('met à jour sans erreur', async () => {
        await expect(
            updateInTable('words', { word_id: 'NewTest' }, { where: 'eq', col: 'word_id', value: 'Test' })
        ).resolves.not.toThrow();
    });

    it('appelle update() avec les bonnes valeurs', async () => {
        await updateInTable('words', { word_id: 'NewTest' }, { where: 'eq', col: 'word_id', value: 'Test' });
