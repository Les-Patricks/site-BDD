...
/**
 * Tests contrat notify.
 */

/** Durees : success generique 4 s, warning 6 s, save success 2,5 s (option). */
...
describe("notify module", () => {
	...
	it("expose notify.success, notify.warning, notify.error et une fonction show commune", async () => {
	...
	it("success (defaut plan) affiche le message puis auto-dismiss apres 4 s", async () => {
		const { notify } = await import("../notify.js");
		notify.success("ok");
	...
	it("success avec durationMs 2500 (chemin type save) disparait apres 2,5 s", async () => {
		const { notify } = await import("../notify.js");
	...
	it("warning (defaut plan) auto-dismiss apres 6 s", async () => {
		const { notify } = await import("../notify.js");
	...
	it("error utilise role alert et aria-live assertive sur l'item", async () => {
		const { notify } = await import("../notify.js");
		notify.error("echec");
		const { body } = teardownDom;
		const region = body.children[body.children.length - 1];
	...
	it("error reste affichee sans auto-dismiss (meme apres un long delai)", async () => {
		const { notify } = await import("../notify.js");
	...
	it("error propose un bouton Fermer avec aria-label ; clic retire la notification", async () => {
		const { notify } = await import("../notify.js");
	...
	it("notify ne delegue pas a window.alert", async () => {
