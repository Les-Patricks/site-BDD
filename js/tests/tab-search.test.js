...
describe("tab search behavior", () => {
	it("filters by normalized substring on root labels", () => {
	...
	it("empty query restores full visibility", () => {
		const result = computeFilterState(
			["Alpha", "Beta", "Gamma"],
			normalizeSearchQuery("   "),
		);
	...
	it("exposes first visible index for loupe/Enter scroll", () => {
		const result = computeFilterState(
			["Alpha", "Beta", "Gamma"],
	...
	it("supports independent filtering per tab context", () => {
	...
	it("works from in-memory labels only", () => {
		const labels = ["A", "B", "C"];
...
describe("initTabSearch robustness", () => {
	beforeEach(() => {
		resetTabSearchModuleStateForTests();
...
describe("refresh keeps filter after list mutation", () => {
	beforeEach(() => {
		resetTabSearchModuleStateForTests();
		document.body.innerHTML = minimalTabPanelsHtml;
		initTabSearch();
...
describe("row label from DOM (family wrapper vs nested words)", () => {
