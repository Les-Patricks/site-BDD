...
describe("bootstrap loading indicator", () => {
	beforeEach(() => {
		vi.resetModules();
		vi.clearAllMocks();
	...
	it("masque le loader en succes et conserve le flux Publish", async () => {
	...
	it("retire le loader aussi en erreur bootstrap (si auth ok)", async () => {
		getSessionMock.mockResolvedValue({ data: { session: { user: {} } } });
		invokeMock.mockResolvedValueOnce({
