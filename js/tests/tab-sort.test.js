...
describe("tab-panel-order sorts lists", () => {
	beforeEach(async () => {
		vi.resetModules();
		vi.clearAllMocks();
	...
	it("clicking Trier par V sorts A->Z then Z->A", async () => {
		...
		orderBtn.click();
		// Tri ne doit pas declencher d'appel reseau.
	...
	it("recherche puis tri => tri sur le sous-ensemble visible", async () => {
	...
	it("apres tri, ouverture accordion cible le bon item", () => {
		const orderBtn = document.querySelector("#wordTab .tab-panel-order");
		expect(orderBtn).toBeTruthy();
	...
	it("tri d'un onglet ne doit pas affecter un autre onglet", async () => {
